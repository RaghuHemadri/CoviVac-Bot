...
def get_main_buttons() -> List[InlineKeyboardButton]:
    return [
        ...
        InlineKeyboardButton("Open Slots", callback_data='cmd: check_slots'),
    ]
...
def start(update: Update, _: CallbackContext) -> None:
    ...
    """
    msg = """Heya peep!! Good Day ;~)
I check for Open Vaccination Slots by PinCode and notify you

For instant check click *Open Slots*.

First time users let me know age preference and pincode"""
...
def get_help_text_short() -> str:
    return """I check for Open Vaccination Slots by PinCode and notify you.

For instant check click *Open Slots*.

First time users let me know age preference and pincode.""" ## noqa
...
def delete_cmd_handler(update: Update, _: CallbackContext):
    ...
    try:
        ...
        update.effective_chat.send_message("Data not existing.")
        return
    ...
    user.save()
    update.effective_chat.send_message("Data Deleted. Click on /start to restart the bot.")
...
def pincode_command(update: Update, _: CallbackContext):
    update.effective_chat.send_message("Enter pincode")
...
def disable_alert_command(update: Update, _: CallbackContext) -> None:
    ...
    user.enabled = False
    user.save()
    update.effective_chat.send_message("Alerts Disabled. Click on /resume to resume the alerts")
...
*{{ c.name }}* {%- if c.fee_type == 'Paid' %}*(Paid)*{%- endif %}:{% for s in c.get_available_sessions() %}
    • {{s.date}}: 
        *Total Vaccines Available:* {{s.capacity}}{%- if display_age %} *({{s.min_age_limit}}+)*{%- endif %}
        *Dose1:* {{s.dose1}}
        *Dose2:* {{s.dose2}}
        *Vaccine Type:* {{s.vaccine}}{% endfor %}
{% endfor %}"""
...
def get_message_header(user: User) -> str:
    return F"Slots available at (pincode: {user.pincode}, for age: {user.age_limit})\n"
...
def check_slots_command(update: Update, ctx: CallbackContext) -> None:
    ...
    if not vaccination_centers:
        update.effective_chat.send_message(
            F"No slots available at (pincode: {user.pincode}, for age: {user.age_limit})")
        return
...
def set_age_preference(update: Update, ctx: CallbackContext) -> None:
    ...
    if user.pincode:
        update.effective_chat.send_message(F"Age preference set to {user.age_limit}",
                                           reply_markup=InlineKeyboardMarkup([[*get_main_buttons()]]))
    else:
        update.effective_chat.send_message(
            F"Age preference set to {user.age_limit}. Please enter pincode to proceed")
...
def set_pincode(update: Update, ctx: CallbackContext) -> None:
    ...
    msg: str = F"Pincode updated to {pincode}. To change send valid pincode " \
               "any time to me."
